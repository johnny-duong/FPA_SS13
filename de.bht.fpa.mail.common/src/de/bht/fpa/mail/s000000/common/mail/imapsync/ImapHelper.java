...
public class ImapHelper {
	...
	private static final EntityManager entityManager = PersistenceManager
			.getInstance().createEntityManagerFactory().createEntityManager();
	...
	public static Account getAccount(String name) {
		Query allAccountQuery = entityManager.createQuery(
				"select a from Account a where a.name = :name").setParameter(
				"name", name);
	 ...
	 */
	public static void syncAllFoldersToAccount(Account account,
			IProgressMonitor monitor) throws SynchronizationException {
		syncAllFoldersToAccount(account, monitor,
				new Hashtable<String, String>(), JAVA_MAIL_IMAPS);
	}
	 ...
	 * @param properties
	 *            a key-value Map to override properties (i.e. for another
	 *            port).
	 * @throws SynchronizationException
	 */
	public static void syncAllFoldersToAccount(Account account,
			IProgressMonitor monitor, Dictionary<String, String> properties,
			String protocol) throws SynchronizationException {
		try {
			...
			try {
				store.connect(account.getHost(), account.getUsername(),
						account.getPassword());
				...
				monitor.beginTask("Syncing " + totalNumberOfFolders
						+ " IMAP Folders", totalNumberOfFolders);
				...
				for (javax.mail.Folder subFolder : imapFolder.list()) {
					...
					Folder syncedFolder = syncFolderInternal(account, monitor,
							(IMAPFolder) subFolder, null);
			...
			throw new SynchronizationException("Synchronization of Account '"
					+ account.getName() + "' failed:", e);
		}
	}

	private static int caculateTotalNumberOfFolders(IMAPFolder imapFolder)
	...
	private static Folder syncFolderInternal(Account account,
			IProgressMonitor monitor, IMAPFolder imapFolder, Folder parent) {

		Folder folder = getFolder(account, imapFolder.getName());
		if (parent != null && !parent.getFolders().contains(folder)) {
			parent.getFolders().add(folder);
			debug("Added Folder " + folder.getFullName() + " to parent "
					+ parent.getFullName());
		}

		long fromUID = folder.getLastUID() + 1;
		debug("Sync IMAP-folder " + imapFolder.getFullName()
				+ " with min UID = " + fromUID);

		try {
			imapFolder.open(javax.mail.Folder.READ_ONLY);
			javax.mail.Message[] messages = imapFolder.getMessagesByUID(
					fromUID, UIDFolder.LASTUID);
			...
			for (javax.mail.Message message : messages) {
				...
				try {
					...
					Message convertedMessage = MessageConverter
							.convertJavaxMessage(imapFolder, message);
					debug("Converted Message ID: " + convertedMessage.getId()
							+ "to: " + convertedMessage.toShortString());
			...
			error("Could not sync folder " + folder.getFullName() + ": "
					+ me.getMessage());
		} finally {
			monitor.worked(1);
			if (imapFolder.isOpen()) {
				try {
					...
					error("Could not close connection to IMAP folder"
		...
		try {
			for (javax.mail.Folder subFolder : imapFolder.list()) {
				syncFolderInternal(account, monitor, (IMAPFolder) subFolder,
						folder);
			}
		} catch (MessagingException e) {
			error("Could not sync sub folders of" + folder.getFullName() + ": "
					+ e.getMessage());
		}
	...
	private static Folder getFolder(Account account, String fullName) {
		Query allFoldersWithFullName = entityManager
				.createQuery(
						"select f from Folder f where f.account.id = :accountId AND f.fullName = :fullName")
